...
public class RiskAnalysisService(IDbContextFactory<TrackRecordDbContext> dbFactory, ICurrentUserAccessor currentUser) : IRiskAnalysisService
{
    /// <summary>
    ...
    /// </summary>
    private static async Task<FunnelData> LoadFunnelAsync(TrackRecordDbContext db, string userId, CancellationToken ct)
    {
        var accounts = await db.TradingAccounts.AsNoTracking()
            .Where(a => a.UserId == userId)
